...
**Structured logging via `log/slog`.** Diagnostics go through the default logger (`slog.Info`, `slog.Debug`) with key-value attrs — never `fmt.Println` for logging. daemonkit is a library: it never configures the global logger; consumers do.
...
**Testing.** Tests live beside the code as `*_test.go`; run them with `task test` (`scripts/test.sh -race -count=1 ./...`) — **never bare `go test`**, see § Testing. Write table-driven tests with strict assertions against specific values, mock the boundaries your code talks to (network, filesystem, clock), and leave the code under test real.
