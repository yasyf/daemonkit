...
func TestSingleEntrantBindsWhenFree(t *testing.T) {
	...
	ln, lock, err := se.Listen(context.Background())
	if err != nil {
...
func TestSingleEntrantRefusesLivePeer(t *testing.T) {
	...
	_, _, err = SingleEntrant{Socket: socket, Evict: refuse(refusal)}.Listen(context.Background())
...
func TestSingleEntrantRefusesStartingPeer(t *testing.T) {
	...
	_, _, err = SingleEntrant{Socket: socket, Evict: noPeer}.Listen(context.Background())
...
func TestSingleEntrantEvictsAndReacquires(t *testing.T) {
	...
	ln, lock, err := SingleEntrant{Socket: socket, Evict: evict, Timeout: 5 * time.Second}.Listen(context.Background())
	if err != nil {
...
func TestSingleEntrantNeverUnlinksLock(t *testing.T) {
	...
	ln, lock, err := SingleEntrant{Socket: socket, Evict: noPeer}.Listen(context.Background())
	if err != nil {
	...
	if _, _, err := (SingleEntrant{Socket: socket, Evict: refuse(errors.New("busy"))}).Listen(context.Background()); err == nil {
...
func TestSingleEntrantListenHonorsContext(t *testing.T) {
	socket := filepath.Join(shortSockDir(t), "m.sock")
	held, err := os.OpenFile(socket+".lock", os.O_CREATE|os.O_RDWR, 0o600)
	if err != nil {
		t.Fatal(err)
	...
	defer cancel()
	start := time.Now()
	_, _, err = SingleEntrant{Socket: socket, Evict: makeWay, Timeout: time.Hour}.Listen(ctx)
	if !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("Listen with a cancelled ctx = %v, want context.DeadlineExceeded", err)
	}
	if !errors.Is(err, ErrLockStillHeld) {
		t.Errorf("error = %v, want it to also wrap ErrLockStillHeld", err)
	}
	if waited := time.Since(start); waited > time.Second {
		t.Fatalf("Listen took %v to honor a 50ms ctx deadline (Timeout was 1h)", waited)
	}
}
