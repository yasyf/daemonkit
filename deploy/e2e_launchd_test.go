...
// TestE2EDeployArchivesALegacyTreeOnARealDeployment is item 7's checkable
// half: a machine carrying the pre-rename .daemonkit-deployment tree — what a
// pre-cut peer left behind — must archive it on first open and converge to a
// ready daemon through a clean re-install, never fail on a record whose fields
// this build no longer has.
func TestE2EDeployArchivesALegacyTreeOnARealDeployment(t *testing.T) {
	f := newE2EFixture(t)

	// What a pre-cut release left: the old directory name, holding a record
	// whose fields readRecord would refuse.
	legacy := filepath.Join(f.root, legacyMetadataDir, e2eBundleName)
	if err := os.MkdirAll(legacy, 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(legacy, "activation.json"), []byte(legacyActivation), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(legacy, "deployment.lock"), nil, 0o600); err != nil {
		t.Fatal(err)
	}

	candidate := f.e2eCandidate("Source1", "1.0.0", "one")
	if _, err := f.deploy.Install(f.e2eCtx(120*time.Second), candidate); err != nil {
		t.Fatalf("Install over a legacy tree = %v", err)
	}
	if fileExists(legacy) {
		t.Errorf("the legacy tree at %s was not archived", legacy)
	}
	archive := legacy + ".bak"
	if !fileExists(archive) {
		t.Fatalf("no archive at %s; the legacy tree was destroyed rather than kept as evidence", archive)
	}
	kept, err := os.ReadFile(filepath.Join(archive, "activation.json"))
	if err != nil {
		t.Fatalf("archived record: %v", err)
	}
	if string(kept) != legacyActivation {
		t.Errorf("the archived record was rewritten rather than preserved")
	}
	t.Logf("legacy tree archived to %s with its record intact", archive)

	activation, err := f.deploy.Activate(f.e2eCtx(180 * time.Second))
	if err != nil {
		t.Fatalf("Activate after the archive = %v\nevents:%s", err, f.events())
	}
	t.Logf("reached ready through a clean re-install: build=%s generation=%d",
		activation.Readiness.Build()[:12], activation.Readiness.Generation())
	if activation.Generation.Version != "1.0.0" {
		t.Errorf("activation generation = %s, want 1.0.0", activation.Generation.Version)
	}

	// A second legacy tree (a downgrade that recreated it, then a re-upgrade)
	// must take the next archive name rather than wedge the deployment.
	if err := os.MkdirAll(legacy, 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if !fileExists(legacy + ".bak.2") {
		t.Errorf("a second legacy tree did not rotate to %s.bak.2", legacy)
	}
	t.Logf("a re-created legacy tree rotated to %s.bak.2 rather than wedging", legacy)
}
