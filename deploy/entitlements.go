...
func DigestEntitlements(payload []byte) (SHA256, error) {
	...
	if _, err := plist.Unmarshal(payload, &value); err != nil {
		return SHA256{}, fmt.Errorf("deploy: parse entitlements: %w", err)
	}
	if value == nil {
		return SHA256{}, errors.New("deploy: entitlements must be a dictionary")
	}
	...
	if err != nil {
		return SHA256{}, fmt.Errorf("deploy: encode entitlements: %w", err)
	}
...
func canonicalEntitlementValue(value any) (any, error) {
	...
	case reflect.Float32, reflect.Float64:
		if math.IsInf(reflected.Float(), 0) || math.IsNaN(reflected.Float()) {
			return nil, errors.New("deploy: entitlement contains non-finite real")
		}
	...
	return nil, fmt.Errorf("deploy: unsupported entitlement value %T", value)
}
...
func decodeEntitlementsOutput(output []byte) (SHA256, error) {
	...
	if start < 0 {
		return SHA256{}, errors.New("deploy: codesign returned no entitlement plist")
	}
