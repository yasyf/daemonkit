...
### Sentinel identity is load-bearing

daemonkit's sentinels cross module boundaries: consumers alias them
(`var ErrLockBusy = proc.ErrLockBusy`) and match with `errors.Is` from other
repos. A moved sentinel lives in exactly one package — never re-`errors.New` an
equivalent — and renaming or re-declaring one silently breaks every `errors.Is`
across the fleet.
...
One package per directory, all at the module root (`proc/`, `service/`, `wire/`,
…) — daemonkit is a library and its packages ARE the deliberate public API;
reach for `internal/` only for machinery no consumer may touch. Order each file:
package doc comment, imports, constants, types, constructors, then methods.
Exported identifiers are the API surface — keep the exported set small and
unexport anything callers don't need.
...
Tests live beside the code as `*_test.go`; run them with
`scripts/test.sh -race -count=1 ./...` — **never bare `go test`** (the harness
caps the per-UID process count so a runaway self-exec spawn fails fast instead
of fork-bombing the machine; see AGENTS.md § Testing).
