...
// spawnErrno reads the failure out of the return value, which is where the
// whole posix_spawn family reports it. That is why this file binds through
// RegisterLibFunc while the proc_pidpath and csops seams do not: nothing here
// reads the thread-local errno, so there is no second call whose value could
