...
func (c CalendarInterval) plistBody() (string, error) {
	...
	for _, f := range fields {
		...
		if *f.value < f.lo || *f.value > f.hi {
			return "", fmt.Errorf("launchd: calendar %s=%d out of range [%d,%d]", f.key, *f.value, f.lo, f.hi)
		}
		fmt.Fprintf(&b, "            <key>%s</key>\n            <integer>%d</integer>\n", f.key, *f.value)
		set++
	}
	if set == 0 {
		return "", fmt.Errorf("launchd: calendar interval has no fields set")
	}
	return b.String(), nil
}
...
func canonicalWatchPaths(paths []string) ([]string, error) {
	...
	for _, p := range paths {
		if !filepath.IsAbs(p) || filepath.Clean(p) != p {
			return nil, fmt.Errorf("launchd: watch path %q is not exact and absolute", p)
		}
