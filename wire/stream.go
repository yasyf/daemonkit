...
var (
	...
	errStreamSealed = errors.New("wire: stream sealed")
)
...
func (s *latestStream[T]) try() (T, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.ready {
		value := s.value
		...
		s.ready = false
		if s.closed {
			return value, true, errStreamClosed
		}
		return value, true, nil
	}
	if s.closed {
		var zero T
		return zero, true, errStreamClosed
	}
	var zero T
...
func (l *latestWriteLane) beginWrite(generation uint64) (uint64, []byte, func(error), error) {
	l.mu.Lock()
	if l.err != nil {
		err := l.err
		l.mu.Unlock()
	...
	if generation < l.desired {
		generation = l.desired
		l.taken = generation
		select {
		case <-l.notify:
		default:
	...
	if l.writing != 0 {
		l.mu.Unlock()
		return 0, nil, nil, ErrFlowControl
	}
	l.writing = generation
	payload := append([]byte(nil), l.payload...)
	l.mu.Unlock()
	return generation, payload, func(err error) {
		l.mu.Lock()
		if l.writing == generation {
			l.writing = 0
		}
		l.completeLocked(generation, err)
		l.mu.Unlock()
	}, nil
}

func (l *latestWriteLane) completeLocked(generation uint64, err error) {
	if l.err != nil {
		return
	}
	if err != nil {
